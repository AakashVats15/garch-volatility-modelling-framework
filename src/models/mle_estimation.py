...
def _neg_loglik_arch(params, model_class, returns):
    ...
    if omega <= 0 or alpha < 0:
        return 1e10
    ll = model_class(omega, alpha).loglik(returns)
...
def _neg_loglik_garch(params, model_class, returns):
    ...
    if omega <= 0 or alpha < 0 or beta < 0 or alpha + beta >= 1:
        return 1e10
    ll = model_class(omega, alpha, beta).loglik(returns)
    return -ll if np.isfinite(ll) else 1e10
...
def fit_arch(model_class, returns, start_params=None):
    ...
    if start_params is None:
        v = np.var(r)
        start_params = np.array([0.1 * v, 0.8])
    ...
    return {"model": model, "params": params, "loglik": ll, "aic": aic, "bic": bic, "success": res.success}

def fit_garch(model_class, returns, start_params=None):
    r = np.asarray(returns, dtype=float)
    if start_params is None:
        v = np.var(r)
        start_params = np.array([0.1 * v, 0.05, 0.9])
